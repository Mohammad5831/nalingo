...
const { findAllCourses } = require('../services/courseService');
...
const getAllUsers = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'لیست کاربران با موفقیت دریافت شد', users: users.users, teachers: users.teachers });
  } catch (error) {
...
const changeUserRole = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'نقش با موفقیت تغییر یافت', updateuser });

  } catch (error) {
    res.status(500).json({ message: 'خطا در تغییر نفش', error: error.message });
  }
};
...
const changeTeacherStatus = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'وضعیت با موفقیت تغییر یافت', updateteacher });

  } catch (error) {
    res.status(500).json({ message: 'خطا در تغییر وضعیت', error: error.message });
  }
}
...
const deleteCourse = async (req, res) => {
  ...
  try {
    // Access check
    await checkUserRole(req.user, ['admin']);
    const course = await existingCourseById(courseUUID)
    if (!course) {
      return res.status(404).json({ message: 'دوره یافت نشد' });
    }
    await course.destroy();
    res.status(204).json({ message: 'دوره با موفقیت حذف شد' });
  } catch (error) {
...
const getAllEnrollments = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'لیست دوره های ثبت نام شده با موفقیت دریافت شد', enrollments });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "خطا در دریافت لیست دوره های ثبت نام شده ثبت‌نام‌ها." });
  }
};
