...
const { checkCourseLevel, checkCourseLanguage, checkCourseSkillType } = require('../utilities/courseUtil');
...
const createCourse = async (req, res) => {
  try {
    await sequelize.transaction(async (t) => {
      ...
      // Checking the accuracy of the language, level and skills entered
      await checkCourseLanguage(language);
      await checkCourseLevel(level);
      await checkCourseSkillType(skillType);
...
const getCourseDetails = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'دوره با موفقیت پیدا شد', course });
  } catch (error) {
