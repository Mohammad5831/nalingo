...
const createTicket = async (req, res) => {
    try {
        ...
        if (userType === ('student' || 'admin')) {
            ...
            return res.status(403).json({ message: 'نقش نامعتبر است' })
        };
        if (!userId) {
        ...
        return res.status(201).json({ message: 'تیکت با موفقیت ثبت شد', ticket });
...
const replyToTicket = async (req, res) => {
    try {
        ...
        if (!updateticket) {
            return res.status(500).json({ message: 'خطا در به‌روزرسانی تیکت' });
        }

        return res.status(200).json({ message: 'پاسخ تیکت با موفقیت ثبت شد', updateticket });
    } catch (error) {
        return res.status(500).json({ message: 'خطا در پاسخ دادن به تیکت', error: error.message });
...
const getAllTickets = async (req, res) => {
    try {
        ...
        if (!tickets) {
            return res.status(404).json({ message: 'تیکتی پیدا نشد' });
        }

        return res.status(200).json({ message: 'لیست تیکت ها با موفقیت دریافت شد', tickets })
    } catch (error) {
        return res.status(500).json({ message: 'خطا در دریافت تیکت ها', error: error.message });
    };
};
