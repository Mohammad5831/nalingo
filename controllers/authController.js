...
const login = async (req, res) => {
  try {
    ...
    return res.status(200).json({success: true, message: 'ورود موفقیت‌آمیز بود.', token: token, role: user.role });
  } catch (error) {
...
const updateUserDetail = async (req, res) => {
  try {
    ...
    return res.status(200).json({success: true, message: 'اطلاعات با موفقیت ویرایش شد', response });
...
const loginTeacher = async (req, res) => {
  try {
    ...
    return res.status(200).json({success: true, message: 'ورود استاد موفقیت‌آمیز بود.', token: token, role: "teacher" });
  } catch (error) {
