...
const checkCourseAccess = async (user, course) => {
    ...
    if (!isAdmin && !isOwner) {
        const error = new Error('شما مجاز به ویرایش این دوره نیستید.');
        error.statusCode = 403;
        throw error;
    }
};
...
const checkChapterAccess = async (user, chapter) => {
    ...
    if (!isAdmin && !isOwner) {
        const error = new Error('شما مجاز به ویرایش این فصل نیستید.');
        error.statusCode = 403;
        throw error;
    }
};
...
const checkEpisodeAccess = async (user, episode) => {
    ...
    if (!isAdmin && !isOwner) {
        const error = new Error('شما مجاز به ویرایش این قسمت نیستید.');
        error.statusCode = 403;
        throw error;
    }
};
